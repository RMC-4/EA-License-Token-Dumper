...
static std::wstring markerPath = fs::temp_directory_path() / L"dll_run_marker.flag";
...
extern "C" __declspec(dllexport) void Run(bool overwrite = true)
{
    try
    {
        ...
        if (overwrite) {
            ...
            bool tokenLineExists = std::regex_search(configText, match, pattern);
            bool tokenDifferent = true;

            if (tokenLineExists) {
                std::string existingToken = match[2].str();
                tokenDifferent = (existingToken != token);
            }

            // Ask user what to do
            int res;
            if (tokenLineExists) {
                res = MessageBoxW(nullptr,
                    L"Token already exists in config.\n\n"
                    L"Yes: Replace the token.\n"
                    L"No: Leave config unchanged.\n"
                    L"Cancel: Remove license (.dlf) file.",
                    L"Update DenuvoToken",
                    MB_YESNOCANCEL | MB_ICONQUESTION);
            }
            else {
                // No existing token: ask if want to continue writing or remove dlf.
                res = MessageBoxW(nullptr,
                    L"No existing token in config.\n\n"
                    L"Yes: Write new token.\n"
                    L"No: Leave config unchanged.\n"
                    L"Cancel: Remove license (.dlf) file.",
                    L"Write DenuvoToken",
                    MB_YESNOCANCEL | MB_ICONQUESTION);
            ...
            if (res == IDCANCEL) {
                ...
                try {
                    if (fs::exists(dlfPath)) {
                        fs::remove(dlfPath);
                        ShowMessage(L"License file removed successfully.", L"Info", MB_ICONINFORMATION);
                    }
                    else {
                        ShowMessage(L"License file not found.", L"Warning", MB_ICONWARNING);
                    }
                }
                catch (...) {
                    ShowMessage(L"Failed to remove license file.", L"Error", MB_ICONERROR);
                }
                return; // End after removal
            }
            else if (res == IDNO) {
                // Leave config untouched
                ShowMessage(L"Config file left unchanged.", L"Info", MB_ICONINFORMATION);
                return;
            }
            else if (res == IDYES) {
                // Write/update token only if token is different or does not exist
                if (!tokenLineExists || tokenDifferent) {
                    std::string replacement = "\"DenuvoToken\" \"" + token + "\"";
                    std::string updated;
                    if (tokenLineExists) {
                        updated = std::regex_replace(configText, pattern, replacement);
                    }
                    else {
                        // Append token line to end of config with newline
                        updated = configText;
                    ...
                    // Commented out success popup to suppress it
                    // ShowMessage(L"DenuvoToken updated!\nBackup created at:\n" + backupPath, L"Success", MB_ICONINFORMATION);
                }
                else {
                    ShowMessage(L"Token already up to date. Config not changed.", L"Info", MB_ICONINFORMATION);
                }
            }

            // Delete all Denuvo_ticket_*.txt files in config dir
            try {
                ...
                for (auto& entry : fs::directory_iterator(configDir)) {
                    if (entry.is_regular_file()) {
                        ...
                        if (filename.size() >= 17 &&
                            filename.substr(0, 13) == L"Denuvo_ticket_" &&
    ...
    catch (const std::exception& ex)
    {
        ...
        // Continue app launch despite error
    }
    catch (...)
    {
